...
#    CONTROLLER_IP_HOST = '172.16.10.1' # assim nao funcionou
    ...
    root1.cmd("ip rule add from 10.123.123.1 table routec1")
    root1.cmd("ip rule add from 10.123.123.2 table routec2")
    root1.cmd("ip rule add from 10.123.123.3 table routec3")
    root1.cmd( 'arp -i root1-eth0 -s 172.16.10.1 8e:49:d6:d3:1e:df' )
    root1.cmd( 'arp -i root1-eth0 -s 172.16.10.2 0e:7b:ac:84:a3:69' )
    root1.cmd( 'arp -i root1-eth0 -s 172.16.10.3 ae:f4:cf:ab:4c:15' )
    root1.cmd('arp -i root1-eth0 -s 172.16.10.4 b2:2c:c7:c1:72:a4')
