...
def enviar_contratos(host_ip, host_port, ip_dst_contrato):
    ...
    for i in contratos:
        if json.loads(i)['contrato']['ip_destino'] == ip_dst_contrato:
...
class Regra:
    def __init__(self, ip_src, ip_dst, porta_dst, tos, banda, prioridade, classe, emprestando):
        ...
        print("src:%s; dst=%s; porta_dst=%d, tos=%s, emprestando=%d" % (self.ip_src, self.ip_dst, self.porta_dst, self.tos, self.emprestando)) 
...
#            print("[getPorta] comparando %s x %s\n" % (i.nome, nomePorta))
            if i.nome == nomePorta:
                return i
...
#a classe, a prioridade e a banda sao os atributos originais do fluxo
        ...
        print("[alocarGBAM-S%s] porta %s, classe %d, banda usada: %d, banda total: %d \n" % (self.nome, str(nomePorta), classe, cU, cT)) 
        ...
        if int(banda) <= cT - cU: #Total - usado > banda necessaria
            ...
            if bandaE >= int(banda):
                ...
                if int(banda) <= cOT - cOU:
                    ...
                    tos = CPT[(str(outraClasse), str(prioridade), str(banda))] # mudou a classe em que ira ser posicionada, entao muda o tos
...
# hardtimeout = 5 segundos # isso eh para evitar problemas com pacotes que sao marcados como best-effort por um contrato nao ter chego a tempo. Assim vou garantir que daqui 5s o controlador possa identifica-lo. PROBLEMA: fluxos geralmente nao duram 5s, mas eh uma abordagem.
        ...
        ##agora todas as portas tem filas
...
#add regra tabela CLASSIFICATION
    def addRegraC(self, ip_src, ip_dst, ip_dscp):
        ...
        print("[addregraC] ipsrc: %s, ipdst: %s, tos: %d\n" % (ip_src, ip_dst, ip_dscp))

        datapath = self.datapath
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser

        #match = parser.OFPMatch(eth_type='0x0800', ipv4_src=ip_src, ipv4_dst=ip_dst) ##acusando erro aqui
        match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP, ip_proto=in_proto.IPPROTO_TCP, ipv4_src="172.16.10.1", ipv4_dst="172.16.10.2")
        
        #erro no actions ou no instruction actions -- testando
        actions = [parser.OFPActionSetField(nw_tos=20)]
...
#        icmph = icmp.icmp(type, 0, 0, echo)
    ...
    def flow_removed_handler(self, ev):
        ...
        switch = Switch.getSwitch(str(dp.id))
        if switch != None:
...
#            print("type = %d" % (pkt_icmp.type))
            if pkt_icmp.type == 15: #request information -> enviar um information reply
                ...
                self.send_icmp(dp,MACC, IPC, src, ip_src, in_port,0,data,1,16,64)
                ...
                for i in switches:
                    if i.nome == str(dpid):
                        out_port = i.getPortaSaida(ip_dst)
                        break

		#ICMP inf reply deve ser respondido pelo switch mais proximo do destino (da borda) - ajustar
            ...
            if pkt_icmp.type==16:
                ...
                enviar_contratos(ip_dst, 4444, "10.123.123.2")#deve ir pela fila de controle
                return 0
                ...
                if cip_src == ip_src and cip_dst == ip_dst:
                    ...
                    for s in switches_rota:
                        if s.nome == str(dpid):
                            ...
                            out_port = s.getPortaSaida(ip_dst)
                            ...
                            #1 criar regra de marcacao/classificacao 
                            s.addRegraC(ip_src, ip_dst, tos)
...
#                    if out_port == 4: #por enquanto somente as portas 4 ligam os switches, entao so elas possuem filas
                            s.alocarGBAM(out_port, ip_src, ip_dst, banda, prioridade, classe)

                     #VOLTA
#                    if in_port == 4:
                            s.alocarGBAM(in_port, ip_dst, ip_src, banda, prioridade,classe)
					
#return do if -> match encontrado
                            return
	    ...
	    #todos os contratos foram checados e nao foi achado correspondencia
            ...
            for s in switches_rota:
                if s.nome == str(dpid):
                    ...
                    s.addRegraC(ip_src, ip_dst, 28)

                    #ida
                    s.alocarGBAM(out_port, ip_src, ip_dst, '', 3, 3)

                    #volta
                    s.alocarGBAM(in_port, ip_dst, ip_src, '', 3, 3)

            return	 
                    
        
            #criar regras para trafego de contratos na rede :: porta 4444 :: serem enviados pela fila de controle classe 4
        
        
                     #IDA
                     #    if out_port == 4:
                     #        s.addRegraF(ip_src, ip_dst, '0b011100', out_port, 6, None,0)
                     #    else:
                     #        s.addRegraF(ip_src, ip_dst, '0b011100', out_port, 0, None,0)

                         #VOLTA
                     #    if in_port == 4:
                     #        s.addRegraF(ip_dst,ip_src, '0b011100', in_port, 6,None,0) 
                     #    else:
                     #        s.addRegraF(ip_dst, ip_src, '0b011100', in_port, 0, None,0)

                         #MARCACAO
                     #    s.addRegraC(ip_src, ip_dst, '0b011100')
