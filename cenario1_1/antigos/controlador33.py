...
CPT[(3,1,'')] = '011100' #best-effort
CPT[(4,2,'1000')] = '011101' #controle
...
#a classe, a prioridade e a banda sao os atributos originais do fluxo
        ...
        if banda <= cT - cU: #Total - usado > banda necessaria
            ...
            self.addRegraF(origem, destino, tos, nomePorta, fila, meter_id)
...
#add regra tabela FORWARD
    def addRegraF(self, ip_src, ip_dst, ip_dscp, out_port, fila, meter_id):
             ...
             for i in contratos:
                     ...
                     for i in switches:
                         if i.nome == str(dpid):
                             ...
                             if out_port == 4: #por enquanto somente as portas 4 ligam os switches, entao so elas possuem filas
                                 ...
                                 i.addRegraF(ip_src, ip_dst, '', out_port,0,None)

                             #VOLTA
                             if in_port == 4:
                                 i.alocarGBAM(in_port, ip_dst, ip_src, banda, prioridade,classe)
                             else:
                                 i.addRegraF(ip_dst, ip_src, '', in_port, 0, None)
                 ...
                 for i in switches:
                     if i.nome == str(dpid):
                         ...
                         #IDA
                         if out_port == 4:
                             i.addRegraF(ip_src, ip_dst, '0b011100', out_port, 6, None)
                         else:
                             i.addRegraF(ip_src, ip_dst, '0b01100', out_port, 0, None)

                         #VOLTA
                         if in_port == 4:
                             i.addRegraF(ip_dst,ip_src, '0b011100', in_port, 6,None) 
                         else:
                             i.addRegraF(ip_dst, ip_src, '0b011100', in_port, 0, None)
