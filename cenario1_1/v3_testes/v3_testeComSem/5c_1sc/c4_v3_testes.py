...
CONTROLADOR_ID = 4
IPC = "10.123.123.4" #IP do root/controlador
MACC = "00:00:00:00:00:08" #MAC do root/controlador
...
#    tcp.bind((socket.gethostbyname(socket.gethostname()),4444))
    ...
    while True:
        ...
        tempo_in = datetime.datetime.now().time()
...
#OBS - os contratos sao armazenados como string, entao para acessa-los como json, eh preciso carregar como json: json.loads(contrato)['contrato']['ip_origem']
        ...
        ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])

        print("[%s] servidor_socket host - fim %f:\n" % (datetime.datetime.now().time(), ttt))
...
#OBS - os contratos sao armazenados como string, entao para acessa-los como json, eh preciso carregar como json: json.loads(contrato)['contrato']['ip_origem']
        ...
        conn.close()
        ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])
        print("[%s] servidor_socket controlador - fim%f:\n" % (datetime.datetime.now().time(), ttt))
...
#    tcp.connect(("10.123.123.2", host_port))
    ...
    ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])
    print("[%s] enviar contrato p/ %s - fim%f:\n" % (datetime.datetime.now().time(), host_ip, ttt))
...
#        #print(ofp_event.__dict__)##printar a classe como um dicionario -> identificar os possiveis eventos
        ...
        if datapath.id == 1:
            ...
            switch.addRede('10.123.123.4',4) #rota para controlador do S2
            switch.addRede('10.123.123.5',4) #rota para controlador do S2
            ...
            switch.addRede('10.10.10.5',4) #rota para controlador do S1
		...
		#switch S2 - dominio C2
            ...
            switch.addRede('10.123.123.3',1)
            switch.addRede('10.123.123.2',1) #rota para controlador do S2
            switch.addRede('10.123.123.1',1) #rota para controlador do S1
            #fic-real
            switch.addRede('10.10.10.5',2)
            switch.addRede('10.10.10.4',5)
            switch.addRede('10.10.10.3',1)
            switch.addRede('10.10.10.2',1) #rota para controlador do S2
            switch.addRede('10.10.10.1',1) #rota para controlador do S1
            #fic-indiv.contr
            switch.addRede('20.40.40.5',2)
            switch.addRede('20.40.40.4',5)
            switch.addRede('20.40.40.3',1)
            switch.addRede('20.40.40.2',1) #rota para controlador do S2
            switch.addRede('20.40.40.1',1) #rota para controlador do S1
            ...
            LISTA_HOSTS['10.10.10.5'] = 5
            LISTA_HOSTS['10.123.123.5'] = 5
            # dizendo qual o primeiro switch da rota para os hosts determinados
            LISTA_HOSTS['172.16.10.1'] = 5
            LISTA_HOSTS['172.16.10.2'] = 5
            LISTA_HOSTS['172.16.10.3'] = 5
            LISTA_HOSTS['172.16.10.4'] = 5
            #
            #conf. rotas
            #hosts
            ...
            #real
            switch.addRede('10.123.123.5',5)
            switch.addRede('10.123.123.4',4)
            switch.addRede('10.123.123.3',4)
            switch.addRede('10.123.123.2',4) #rota para controlador do S2
            switch.addRede('10.123.123.1',4) #rota para controlador do S1
            #fic-real
            switch.addRede('10.10.10.5',5)
            switch.addRede('10.10.10.4',4)
            switch.addRede('10.10.10.3',4)
            switch.addRede('10.10.10.2',4) #rota para controlador do S2
            switch.addRede('10.10.10.1',4) #rota para controlador do S1
            #fic-indiv.contr
            switch.addRede('20.50.50.5',5)
            switch.addRede('20.50.50.4',4)
            switch.addRede('20.50.50.3',4)
            switch.addRede('20.50.50.2',4) #rota para controlador do S2
            switch.addRede('20.50.50.1',4) #rota para controlador do S1
...
####### etapa 5 - reijetar icmp 15
    ### SEGUINDO O ICMP 15 inf. req. - injetar pelo ultimo switch da  rota
        #obtendo a rota entre src e destino, assim como era antes
                ...
                ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])
                print("[%s] tratamento ICMP 15 - fim %f:\n" % (datetime.datetime.now().time(), ttt))
    ...
    #criar regras de encaminhamento: switches da rota
                    ...
                    ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])
                    print("[%s] tratamento ICMP 16 - controlador destino - fim %f:\n" % (datetime.datetime.now().time(), ttt))
          ...
          ###### (ii) esse controlador nao eh o controlador destino - logo criar as regras de marcacao e encaminhamento para passar os contratos
                ...
                ttt = float(str(datetime.datetime.now().time()).split(':')[2]) - float(str(tempo_in).split(':')[2])
                print("[%s]  tratamento ICMP 16 - controlador da rota - fim %f:\n" % (datetime.datetime.now().time(), ttt))
