...
#    for i in range(6, 11):
    for i in range(4,7):
        ...
        print("Setting h"+str(i) + " - ip: 172.16.20."+str(i)+"/24")
...
def criarTopo():
  ...
  s2.cmd("ip addr add 172.16.20.24/24 dev s2-eth1") #interface s2-h4
  s2.cmd("ip addr add 172.16.20.25/24 dev s2-eth2") #interface s2-h5
  s2.cmd("ip addr add 172.16.20.26/24 dev s2-eth3") #interface s2-h6
  ...
  s1.cmd("ifconfig s1-eth4 hw ether 0A:0A:0A:0A:0A:00")
  ...
  s2.cmd("ifconfig s2-eth4 hw ether 02:02:02:02:02:00")
